...
import {fmtFor,gameOf} from "../../shared/rules.js";

/* ---------- podium ---------- */
function renderPodium(ord){
  const win=ord[0],lose=ord[ord.length-1],low=gameOf(G).lowestWins;
  /* "calls landed" is a Judgement statistic — Declare has no call to land — so a
     points-only game gets the rounds played instead of a made-bid tally. */
  ...
  $("podium").hidden=false;
  $("podium").innerHTML=`<article class="pcard win">${crown}<div class="tag">Winner · ${low?"lowest":"highest"} total</div><h2 class="title">Sultan-e-Hind</h2><p class="name ${win.navy?'navy':''}">${win.n}</p><div class="big ${win.navy?'navy':''}">${fmtFor(G,win.tot)}</div><div class="sub">${ws.hits} of ${N} calls landed · longest run ${ws.best} · finished ${Math.abs(win.tot-ord[1].tot)} clear of ${ord[1].n}.</div></article>
  <article class="pcard lose">${slip}<div class="tag">Last place · ${low?"highest":"lowest"} total</div><h2 class="title">Gaddar</h2><p class="name ${lose.navy?'navy':''}">${lose.n}</p><div class="big ${lose.navy?'navy':''}">${fmtFor(G,lose.tot)}</div><div class="sub">${ls.hits} of ${N} calls landed · longest run ${ls.best} · ${Math.abs(lose.tot-ord[ord.length-2].tot)} behind ${ord[ord.length-2].n}.</div></article>`;
}
